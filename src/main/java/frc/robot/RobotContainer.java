...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_defaultDriveCommand = Commands.simpleArcadeDrive(m_drivetrain, m_joystickMap.get(JoystickCommand.MOVE),
        m_joystickMap.get(JoystickCommand.TURN));

    m_drivetrain.setDefaultCommand(Commands.simpleArcadeDrive(m_drivetrain, m_joystickMap.get(JoystickCommand.MOVE),
        m_joystickMap.get(JoystickCommand.TURN)));
    // m_intake.setDefaultCommand(Commands.runIntakeExtender_Temp(m_intake,
    // m_joystickMap.get(JoystickCommand.MANUAL_RUN_INTAKE_EXTENDER)));
    ...
    // m_pathPath) : null;
    m_autonomousCommand = Commands.autoCommand(m_shooter, m_queue, m_queueSpeed, m_turret, m_limelight, m_magazine,
        m_magazineSpeed, MagazinePowerCellCounter.getCount(), m_drivetrain, -1.0);
  }
  ...
  private void putButtons() {
    ...
    // m_buttonMap.put(ButtonCommand.BOOST_SPEED,
    // m_controlBoard.extreme.joystickTopRight);
    // m_buttonMap.put(ButtonCommand.CUT_SPEED,
    // m_controlBoard.extreme.joystickBottomRight);




    m_buttonMap.put(ButtonCommand.RAISE_CLIMBERS, m_controlBoard.extreme.baseMiddleRight);
    m_buttonMap.put(ButtonCommand.LOWER_CLIMBERS, m_controlBoard.extreme.baseMiddleLeft);



    m_buttonMap.put(ButtonCommand.EXTEND_LEFT_CLIMBER, m_controlBoard.extreme.joystickTopLeft);
    m_buttonMap.put(ButtonCommand.RETRACT_LEFT_CLIMBER, m_controlBoard.extreme.joystickBottomLeft);



    m_buttonMap.put(ButtonCommand.EXTEND_RIGHT_CLIMBER, m_controlBoard.extreme.joystickTopRight);
    m_buttonMap.put(ButtonCommand.RETRACT_RIGHT_CLIMBER, m_controlBoard.extreme.joystickBottomRight);
    ...
    // m_buttonMap.put(ButtonCommand.INTAKE_EXTENDER_MOTION_MAGIC,
    // m_controlBoard.extreme.joystickTopLeft);
    ...
    // m_buttonMap.put(ButtonCommand.TURRET_TESTING_MOTION_MAGIC,
    // m_controlBoard.extreme.baseFrontLeft);

    // m_buttonMap.put(ButtonCommand.AUTO_REFILL_QUEUE,
    // m_controlBoard.extreme.baseBackLeft);
    // m_buttonMap.put(ButtonCommand.AUTO_RUN_SHOOTER,
    // m_controlBoard.extreme.baseMiddleRight);
    // m_buttonMap.put(ButtonCommand.AUTO_SHOOT,
    // m_controlBoard.extreme.baseMiddleLeft);
    // m_buttonMap.put(ButtonCommand.AUTONOMOUS,
    // m_controlBoard.extreme.baseFrontLeft);
    ...
    m_joystickMap.put(JoystickCommand.MANUAL_RUN_INTAKE_EXTENDER,
        () -> -m_controlBoard.extreme.getStickY() * m_intakeExtenderSpeed);
    ...
    m_joystickMap.put(JoystickCommand.MANUAL_RUN_SHOOTER_HOOD,
        () -> m_controlBoard.extreme.getPOVY() * m_shooterHoodSpeed);
  ...
  private void configureButtonBindings() {
    ...
    m_buttonMap.get(ButtonCommand.MANUAL_RUN_INTAKE_REVERSE).whileHeld(Commands.runIntake(m_intake, m_intakeReverseSpeed));
    ...
    // m_buttonMap.get(ButtonCommand.BOOST_SPEED).whenPressed(Commands.boostSpeed(m_shooter,
    // 0.1));
    // m_buttonMap.get(ButtonCommand.CUT_SPEED).whenPressed(Commands.boostSpeed(m_shooter,
    // -0.1));
    ...
    m_buttonMap.get(ButtonCommand.MOVE_POWER_CELLS_REVERSE)
        .whileHeld(Commands.runIntake(m_intake, m_intakeReverseSpeed).alongWith(
            Commands.runMagazine(m_magazine, -m_magazineSpeed).alongWith(Commands.runQueue(m_queue, -m_queueSpeed))));

    m_buttonMap.get(ButtonCommand.SHOOT_BALL).whileHeld(Commands.shootBallsAndAim(m_shooter, m_queue, m_queueSpeed,
    ...
    m_buttonMap.get(ButtonCommand.EXTEND_LEFT_CLIMBER)
        .whenPressed(Commands.extendLeftClimber(m_climber, m_climberSpeed))
        .whenReleased(Commands.extendLeftClimber(m_climber, 0.0));

    m_buttonMap.get(ButtonCommand.RETRACT_LEFT_CLIMBER)
        .whenPressed(Commands.extendLeftClimber(m_climber, -m_climberSpeed))
        .whenReleased(Commands.extendLeftClimber(m_climber, 0.0));

    m_buttonMap.get(ButtonCommand.EXTEND_RIGHT_CLIMBER)
        .whenPressed(Commands.extendRightClimber(m_climber, m_climberSpeed))
        .whenReleased(Commands.extendRightClimber(m_climber, 0.0));

    m_buttonMap.get(ButtonCommand.RETRACT_RIGHT_CLIMBER)
        .whenPressed(Commands.extendRightClimber(m_climber, -m_climberSpeed))
        .whenReleased(Commands.extendRightClimber(m_climber, 0.0));

    m_buttonMap.get(ButtonCommand.CLIMB).whileHeld(Commands.Climb(m_climber ,m_drivetrain, 0.2, 0.2));

    // everything else

    m_buttonMap.get(ButtonCommand.SHIFT_DRIVETRAIN).whenPressed(Commands.drivetrainSetLowGear(m_drivetrain, true))
    ...
    // m_buttonMap.get(ButtonCommand.EXTEND_INTAKE).whenPressed(Commands.setIntakeExtended(m_intake,
    // true));
    // m_buttonMap.get(ButtonCommand.RETRACT_INTAKE).whenPressed(Commands.setIntakeExtended(m_intake,
    // false));

    // Start/stop intaking
    // m_buttonMap.get(ButtonCommand.MANUAL_RUN_INTAKE_REVERSE).whileHeld(Commands.runIntake(m_intake,
    // -m_intakeSpeed));

    // Add auto refill qeuue

    m_buttonMap.get(ButtonCommand.MANUAL_RUN_SHOOTER)
    ...
    // m_buttonMap.get(ButtonCommand.AUTO_FEED_SHOOTER).whenPressed(new
    // InstantCommand(() -> {
    // m_autoFeedShooterEnabled = !m_autoFeedShooterEnabled;
    // if (m_autoFeedShooterEnabled) {
    // m_queue.setDefaultCommand(Commands.autoFeedShooter(m_queue, m_queueSpeed,
    // m_magazinePowerCellCounter::getCount));
    // } else {
  ...
  public void setDriveType(DriveType driveType) {
    if (m_drivetrainEnabled) {
      ...
      case SIMPLE_ARCADE_DRIVE:
        driveCommand = Commands.simpleArcadeDrive(drivetrain, m_joystickMap.get(JoystickCommand.MOVE),
            m_joystickMap.get(JoystickCommand.TURN));
        break;
      case VELOCITY_ARCADE_DRIVE:
        driveCommand = Commands.velocityArcadeDrive(drivetrain, m_joystickMap.get(JoystickCommand.MOVE),
            m_joystickMap.get(JoystickCommand.TURN));
        break;
      case ACCELERATION_LIMITED_SIMPLE_ARCADE_DRIVE:
        driveCommand = Commands.accelerationLimitedSimpleArcadeDrive(drivetrain,
            m_joystickMap.get(JoystickCommand.MOVE), m_joystickMap.get(JoystickCommand.TURN), 2, 3);
        break;
      case ACCELERATION_LIMITED_VELOCITY_ARCADE_DRIVE:
        driveCommand = Commands.accelerationLimitedVelocityArcadeDrive(drivetrain,
            m_joystickMap.get(JoystickCommand.MOVE), m_joystickMap.get(JoystickCommand.TURN), 2, 3);
        break;
      default:
  ...
  public boolean getIntakeExtended() {
    ...
    if (extendButton)
      intakeExtended = extendButton;
    else if (retractButton)
      intakeExtended = !retractButton;
  ...
  public boolean runExtenderMotionMagic() {
    ...
    // m_buttonMap.get(ButtonCommand.INTAKE_EXTENDER_MOTION_MAGIC).get();
    // if (toggleButton && !extenderMotionMagicButtonState) {motionMagic =
    // !motionMagic;}
  ...
  public Command getAutonomousCommand() {
    ...
    // return drivetrainEnabled ? Commands.followPath(m_drivetrain,
    // "test.wpilib.json") : null;
  }
  ...
  /**
   * saves all the properties in properties files in subsystems. should be called
   * in disabled init and any other time nessescary.
   * <p>
   * <B> <I> subsystesm must run their property saving methods here for them to
   * save unless theyre called elswhere
   */
  public void saveAllProperties() {
    if (m_drivetrainEnabled)
      m_drivetrain.saveProperties();
    if (m_intakeEnabled)
      m_intake.saveProperties();
    if (m_shooterEnabled)
      m_shooter.saveProperties();
    if (m_turretEnabled)
      m_turret.saveProperties();
  }
}
